...
int main()
{
	...
	{
		if (64 < a[i] && a[i] < 91)
			a[i] += 32;                 // 대문자면 소문자로 만들어줌
	}
	for (int i = 0; i < b.size(); i++)
	{
		if (64 < b[i] && b[i] < 91)
			b[i] += 32;                 // 대문자면 소문자로 만들어줌
	}
	
	for (int i = 0; i < a.size(); i++)
	{
		array[a[i]]++;  // 해당하는 아스키코드 값을 배열의 순서에 더해줌
	}
	for (int i = 0; i < b.size(); i++)
	{
		array[b[i]]++;  // 해당하는 아스키코드 값을 배열의 순서에 더해줌
	}
	
	for (int i = 97; i < 123; i++) {
		if (array[i]% 2!=0 && array[i] != 0)  // 짝수가 아니거나 0이 아니면 무조건 NO!
		{
